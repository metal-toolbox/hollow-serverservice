...
func testServerComponentToManyComponentFirmwares(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, firmwareDBTypes, false, firmwareColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, firmwareDBTypes, false, firmwareColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.ComponentFirmwares().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := ServerComponentSlice{&a}
	if err = a.L.LoadComponentFirmwares(ctx, tx, false, (*[]*ServerComponent)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ComponentFirmwares); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.ComponentFirmwares = nil
	if err = a.L.LoadComponentFirmwares(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.ComponentFirmwares); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testServerComponentToManyAddOpComponentFirmwares(t *testing.T) {
	var err error
	...
	var a ServerComponent
	var b, c, d, e Firmware

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, serverComponentDBTypes, false, strmangle.SetComplement(serverComponentPrimaryKeyColumns, serverComponentColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*Firmware{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, firmwareDBTypes, false, strmangle.SetComplement(firmwarePrimaryKeyColumns, firmwareColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Firmware{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddComponentFirmwares(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.ComponentFirmwares[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.ComponentFirmwares[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.ComponentFirmwares().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
